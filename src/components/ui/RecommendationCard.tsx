...
const OPTIONS: RecommendationOption[] = [
  {
    ...
    body: (
      <>
        Reorder waffle cones from <EntityChip name="Cone King" /> with lead time <ValuePill tone="green">7 days</ValuePill>
      </>
    ),
    ...
    <div className="w-full max-w-95 overflow-hidden rounded-card bg-surface shadow-card">
      ...
      <div className="primitive-card-footer flex items-center justify-between gap-3 bg-surface">
        ...
        <span className="-mr-0.5 flex items-center gap-2">
          <Button variant="secondary" size="sm" aria-expanded={open} onClick={() => setOpen((current) => !current)} className="px-2.5 text-[12.5px]">
            {t.alternatives}
          </Button>
          <Button variant={accepted ? "success" : active.ctaVariant} size="sm" onClick={() => setAccepted(true)} className="text-[12.5px]">
