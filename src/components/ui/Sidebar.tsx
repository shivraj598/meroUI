...
/* ─────────────────────────────────────────────────────────
 * SIDEBAR NAV — stub icons (replace @central-icons-react)
...
function StubIcon({ size = 18, label = "•" }: { size?: number; label?: string }) {
  return (
    <span
      aria-hidden
      className="inline-flex items-center justify-center rounded-sm bg-zinc-100 text-[10px] font-medium text-zinc-600"
...
const IconArrowBoxLeft = (p: { size: number }) => <StubIcon size={p.size} label="→" />;
const IconCheckmark1Small = (p: { size: number }) => <StubIcon size={p.size} label="✓" />;
const IconChevronDownSmall = (p: { size: number }) => <StubIcon size={p.size} label="⌄" />;
const IconCrossSmall = (p: { size: number }) => <StubIcon size={p.size} label="×" />;
const IconEditBig = (p: { size: number }) => <StubIcon size={p.size} label="✎" />;
const IconHome = (p: { size: number }) => <StubIcon size={p.size} label="⌂" />;
const IconMagnifyingGlass = (p: { size: number }) => <StubIcon size={p.size} label="⌕" />;
const IconPlusMedium = (p: { size: number }) => <StubIcon size={p.size} label="+" />;
const IconPopsicle2 = (p: { size: number }) => <StubIcon size={p.size} label="◈" />;
const IconSettingsGear1 = (p: { size: number }) => <StubIcon size={p.size} label="⚙" />;
const IconSidebarLeftArrow = (p: { size: number; className?: string }) => <span className={p.className}><StubIcon size={p.size} label="‹" /></span>;
const IconUserAdd = (p: { size: number }) => <StubIcon size={p.size} label="+" />;
  ...
  return (
    <button
      ...
      className={`sidebar-row relative z-10 mx-2 flex h-8 items-center rounded-[8px] px-2 text-left
        transition-[width,background-color,color,transform] duration-150 active:scale-[0.98] hover:bg-hover
        ${active ? "bg-hover-2 group-hover/glide:bg-transparent" : "hover:bg-hover-2"}`}
    >
...
function WorkspaceMenu({
  ...
  return createPortal(
    ...
    >
      <GlideMenu rowSelector="[data-menu-row]" className="flex flex-col gap-px" highlightClassName="inset-x-0 rounded-[8px] bg-hover-2">
        <button
          data-menu-row
          type="button"
          onClick={onClose}
          className="relative z-10 flex h-10 w-full items-center gap-1.5 rounded-[8px] px-2 text-left hover:bg-hover-2"
        >
        ...
        {[
          ...
          <button
            ...
            type="button"
            onClick={onClose}
            className="relative z-10 flex h-9 w-full items-center gap-1.5 rounded-[8px] px-2 text-left hover:bg-hover-2"
          >
...
export default function SidebarNav({
  ...
  return (
    <aside
      ...
      className={`relative flex shrink-0 overflow-hidden border-r border-line bg-surface transition-[width] ${fill ? "h-full" : "h-[600px]"} ${className}`}
      style={
        {
            ...
            {visibleRecents.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`sidebar-row relative z-10 mx-2 flex h-8 items-center rounded-[8px] px-2 text-left transition-[width,background-color,color,transform] duration-150 active:scale-[0.98] hover:bg-hover-2 ${active ? "bg-hover-2 group-hover/glide:bg-transparent" : ""}`}
                >
                  <span className={`sidebar-copy min-w-0 flex-1 truncate text-[14px] font-medium ${active ? "text-ink" : "text-ink-2"}`}>{item.label}</span>
                </button>
              );
            })}
            {query && visibleRecents.length === 0 && <div className="sidebar-copy mx-2 px-2 py-2 text-[12.5px] text-ink-3">No chats found</div>}
          </GlideGroup>
        </div>
